...
class TSLNormalize {
  ...
  constructor(
    ...
    this.weightNode = storage(this.weightAttribute, 'float', hiddenSize)
      .toReadOnly()
      .setName(options.name ? `${options.name}Weight` : 'LLMLayerNormWeight');
    this.biasNode = storage(this.biasAttribute, 'float', hiddenSize)
      .toReadOnly()
      .setName(options.name ? `${options.name}Bias` : 'LLMLayerNormBias');
    this.outputNode = storage(this.outputAttribute, 'float', hiddenSize).setName(
      options.name ? `${options.name}Output` : 'LLMLayerNormOutput',
    );
