...
import type { Architecture, DecoderRecipe, HFModelBundle, HuggingFaceConfig, LoaderOptions, TensorMap, Tokenizer } from './types.js';
...
async function loadTokenizer( root: string, recipe: DecoderRecipe, config: HuggingFaceConfig, options: LoaderOptions, report: ( message: string ) => Promise<void> ): Promise<Tokenizer> {
	...
	if ( recipe.tokenizer === 'qwen' ) {
		...
		try {
			...
			const tokenizerConfig = await fetchJSON<{ added_tokens_decoder?: Record<string, { content: string }> }>( `${ root }tokenizer_config.json`, 'HFModelBundle' );
