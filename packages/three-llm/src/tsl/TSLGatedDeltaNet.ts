...
import type { KernelOptions, QwenDeltaWeights, Renderer, TslNode } from '../types.js';
...
class TSLGatedDeltaNet {
  ...
  bProj: TSLLinear;
  aProj: TSLLinear;
  convStateAttribute: StorageBufferAttribute;
  convOutAttribute: StorageBufferAttribute;
  queryAttribute: StorageBufferAttribute;
  keyAttribute: StorageBufferAttribute;
  valueAttribute: StorageBufferAttribute;
  recurrentAttribute: StorageBufferAttribute;
  mixedAttribute: StorageBufferAttribute;
  outputAttribute: StorageBufferAttribute;
  convStateNode: TslNode;
  convOutNode: TslNode;
  queryNode: TslNode;
  keyNode: TslNode;
  valueNode: TslNode;
  recurrentNode: TslNode;
  mixedNode: TslNode;
  normWeightAttribute: StorageBufferAttribute;
  normWeightNode: TslNode;
  aLogAttribute: StorageBufferAttribute;
  dtBiasAttribute: StorageBufferAttribute;
  convWeightAttribute: StorageBufferAttribute;
  aLogNode: TslNode;
  dtBiasNode: TslNode;
  convWeightNode: TslNode;
  decayAttribute: StorageBufferAttribute;
  betaAttribute: StorageBufferAttribute;
  decayNode: TslNode;
  betaNode: TslNode;
  outProj: TSLLinear;
  outputNode: TslNode;
  convComputeNode: TslNode;
  prepareComputeNode: TslNode;
  normQKComputeNode: TslNode;
  decayComputeNode: TslNode;
  deltaComputeNode: TslNode;
  normComputeNode: TslNode;
  computeNodes: TslNode[];

  constructor(inputNode: TslNode, weights: QwenDeltaWeights, options: GatedDeltaNetOptions = {}) {
    ...
    this.convStateNode = storage(this.convStateAttribute, 'float', this.convDim * this.kernelSize).setName(
      options.name ? `${options.name}ConvState` : 'LLMDeltaConvState',
    );
    this.convOutNode = storage(this.convOutAttribute, 'float', this.convDim).setName(
      options.name ? `${options.name}ConvOut` : 'LLMDeltaConvOut',
    );
    this.queryNode = storage(this.queryAttribute, 'float', this.numVHeads * this.keyDim).setName(
      options.name ? `${options.name}Query` : 'LLMDeltaQuery',
    );
    this.keyNode = storage(this.keyAttribute, 'float', this.numVHeads * this.keyDim).setName(
      options.name ? `${options.name}Key` : 'LLMDeltaKey',
    );
    this.valueNode = storage(this.valueAttribute, 'float', this.valueSize).setName(
      options.name ? `${options.name}Value` : 'LLMDeltaValue',
    );
    this.recurrentNode = storage(this.recurrentAttribute, 'float', this.stateSize).setName(
      options.name ? `${options.name}State` : 'LLMDeltaState',
    );
    this.mixedNode = storage(this.mixedAttribute, 'float', this.valueSize).setName(
      options.name ? `${options.name}Mixed` : 'LLMDeltaMixed',
    );
    this.normWeightAttribute = new StorageBufferAttribute(weights.normWeight, 1);
    this.normWeightNode = storage(this.normWeightAttribute, 'float', this.valueDim)
      .toReadOnly()
      .setName(options.name ? `${options.name}Norm` : 'LLMDeltaNorm');
    ...
    this.aLogNode = storage(this.aLogAttribute, 'float', this.numVHeads)
      .toReadOnly()
      .setName(options.name ? `${options.name}ALog` : 'LLMDeltaALog');
    this.dtBiasNode = storage(this.dtBiasAttribute, 'float', this.numVHeads)
      .toReadOnly()
      .setName(options.name ? `${options.name}DtBias` : 'LLMDeltaDtBias');
    this.convWeightNode = storage(this.convWeightAttribute, 'float', this.convDim * this.kernelSize)
      .toReadOnly()
      .setName(options.name ? `${options.name}ConvW` : 'LLMDeltaConvW');
    ...
    this.decayNode = storage(this.decayAttribute, 'float', this.numVHeads).setName(
      options.name ? `${options.name}Decay` : 'LLMDeltaDecay',
    );
    this.betaNode = storage(this.betaAttribute, 'float', this.numVHeads).setName(
      options.name ? `${options.name}Beta` : 'LLMDeltaBeta',
    );
  ...
  createConvNode(name?: string) {
    ...
    return Fn(() => {
      ...
      If(channel.lessThan(uint(convDim)), () => {
        ...
        Loop({ start: uint(1), end: uint(kernelSize), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
          sum.addAssign(
            convWeightNode.element(weightOffset.add(i.sub(uint(1)))).mul(convStateNode.element(stateOffset.add(i))),
          );
        });
        ...
        Loop({ start: uint(0), end: uint(kernelSize - 1), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
      ...
      .compute(convDim, [workgroupSize])
      .setName(name ? `${name}Conv` : 'LLMDeltaConv');
  }

  createPrepareNode(name?: string) {
    const { keySize, valueSize, keyDim, numVHeads, repeat, convOutNode, queryNode, keyNode, valueNode, workgroupSize } =
      this;
    ...
    return Fn(() => {
      ...
      .compute(Math.max(prepared, valueSize), [workgroupSize])
      .setName(name ? `${name}Prepare` : 'LLMDeltaPrepare');
  }

  createNormQKNode(name?: string) {
    ...
    return Fn(() => {
      ...
      If(head.lessThan(uint(numVHeads)), () => {
        ...
        Loop({ start: uint(0), end: uint(keyDim), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
      ...
      .compute(numVHeads, [workgroupSize])
      .setName(name ? `${name}NormQK` : 'LLMDeltaNormQK');
  }

  createDecayNode(name?: string) {
    ...
    return Fn(() => {
      ...
      .compute(numVHeads, [workgroupSize])
      .setName(name ? `${name}Decay` : 'LLMDeltaDecay');
  }

  createDeltaNode(name?: string) {
    const {
      numVHeads,
      keyDim,
      valueDim,
    ...
    return Fn(() => {
      ...
      If(index.lessThan(uint(count)), () => {
        ...
        for (let start = 0; start < keyDim; start += chunk) {
          ...
          (Loop as TslNode)(
            { start: uint(start), end: uint(end), type: 'uint', condition: '<', name: `kA${start}` },
            (vars: Record<string, TslNode>) => {
        ...
        const delta = valueNode
          .element(head.mul(uint(valueDim)).add(v))
          .sub(kvMem)
          .mul(beta);
        ...
        for (let start = 0; start < keyDim; start += chunk) {
          ...
          (Loop as TslNode)(
            { start: uint(start), end: uint(end), type: 'uint', condition: '<', name: `kB${start}` },
            (vars: Record<string, TslNode>) => {
              const k = vars[`kB${start}`];
              const sIndex = stateOff.add(k.mul(uint(valueDim))).add(v);
              recurrentNode
                .element(sIndex)
                .assign(recurrentNode.element(sIndex).add(keyNode.element(qOff.add(k)).mul(delta)));
      ...
      .setName(name ? `${name}Delta` : 'LLMDeltaRule');
  }

  createNormNode(name?: string) {
    ...
    return Fn(() => {
      ...
      If(head.lessThan(uint(numVHeads)), () => {
        ...
        Loop({ start: uint(0), end: uint(valueDim), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
        ...
        Loop({ start: uint(0), end: uint(valueDim), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
          ...
          mixedNode
            .element(index)
            .assign(mixedNode.element(index).mul(invRms).mul(normWeightNode.element(i)).mul(silu));
        });
      });
    })()
      .compute(numVHeads, [workgroupSize])
      .setName(name ? `${name}Norm` : 'LLMDeltaGatedNorm');
  }
