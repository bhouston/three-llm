...
class TSLSplitHeadGate {
  ...
  constructor(packedNode: TslNode, headCount: number, headDim: number, options: KernelOptions = {}) {
    ...
    this.queryNode = storage(this.queryAttribute, 'float', this.qSize).setName(
      options.name ? `${options.name}Query` : 'LLMSplitQuery',
    );
    this.gateNode = storage(this.gateAttribute, 'float', this.qSize).setName(
      options.name ? `${options.name}Gate` : 'LLMSplitGate',
    );
    ...
    this.computeNode = Fn(() => {
      ...
      .compute(this.qSize, [options.workgroupSize || 64])
      .setName(options.name || 'LLMSplitHeadGate');
  }
