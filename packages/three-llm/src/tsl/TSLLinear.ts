...
class TSLLinear {
  ...
  constructor(
    ...
    this.weightNode = storage(this.weightAttribute, 'float', weightArray.length)
      .toReadOnly()
      .setName(options.name ? `${options.name}Weight` : 'LLMLinearWeight');
    this.biasNode = storage(this.biasAttribute, 'float', outputSize)
      .toReadOnly()
      .setName(options.name ? `${options.name}Bias` : 'LLMLinearBias');
    this.outputNode = storage(this.outputAttribute, 'float', outputSize).setName(
      options.name ? `${options.name}Output` : 'LLMLinearOutput',
    );
