...
import { Fn, If, Loop, cos, exp, float, instanceIndex, inversesqrt, sin, storage, uint, uniform } from 'three/tsl';

import type { AttentionKernelOptions, ComputeNode, Renderer, TslNode } from '../types.js';
...
class TSLAttention {
  ...
  ropeFrequencyNode: TslNode;
  ropeAttentionFactor: number;
  ...
  vNorm: boolean;
  sharedKV: boolean;
  gateNode: TslNode | null;
  qNormNode: TslNode | null;
  kNormNode: TslNode | null;
  qNormAttribute?: StorageBufferAttribute;
  kNormAttribute?: StorageBufferAttribute;
  position: TslNode;
  positionUniform: TslNode | null;
  ...
  scoreAttribute: StorageBufferAttribute;
  queryAttribute: StorageBufferAttribute;
  outputAttribute: StorageBufferAttribute;
  scoreNode: TslNode;
  queryNode: TslNode;
  outputNode: TslNode;
  copyComputeNode: ComputeNode | null;
  queryComputeNode: ComputeNode;
  scoreComputeNode: ComputeNode;
  ...
  constructor(
    ...
    const params =
      this.ropeTheta > 0 && this.rotaryDim > 0
        ? ropeParameters(this.ropeFreqDim, this.ropeTheta, options.ropeScaling ?? options.yarn)
        : { invFreq: new Float32Array(1), attentionFactor: 1 };
    this.ropeFrequencyNode = storage(
      new StorageBufferAttribute(params.invFreq, 1),
      'float',
      params.invFreq.length,
    ).toReadOnly();
    this.ropeAttentionFactor = params.attentionFactor;
    ...
    if (options.qNormWeight) {
      ...
      this.qNormNode = storage(this.qNormAttribute, 'float', this.headDim)
        .toReadOnly()
        .setName(options.name ? `${options.name}QNorm` : 'LLMQNorm');
    }

    if (options.kNormWeight) {
      this.kNormAttribute = new StorageBufferAttribute(options.kNormWeight, 1);
      this.kNormNode = storage(this.kNormAttribute, 'float', this.headDim)
        .toReadOnly()
        .setName(options.name ? `${options.name}KNorm` : 'LLMKNorm');
    }
    ...
    if (shared) {
      ...
      this.keyCacheNode = storage(this.keyCacheAttribute, 'float', cacheSize).setName(
        options.name ? `${options.name}KeyCache` : 'LLMKeyCache',
      );
      this.valueCacheNode = storage(this.valueCacheAttribute, 'float', cacheSize).setName(
        options.name ? `${options.name}ValueCache` : 'LLMValueCache',
      );
    }
    ...
    this.scoreNode = storage(this.scoreAttribute, 'float', scoreSize).setName(
      options.name ? `${options.name}Scores` : 'LLMAttentionScores',
    );
    this.queryNode = storage(this.queryAttribute, 'float', this.qSize).setName(
      options.name ? `${options.name}Query` : 'LLMAttentionQuery',
    );
    this.outputNode = storage(this.outputAttribute, 'float', this.qSize).setName(
      options.name ? `${options.name}Output` : 'LLMAttentionOutput',
    );

    this.copyComputeNode = this.sharedKV
      ? null
      : this.createCopyComputeNode(options.name ? `${options.name}CopyKV` : 'LLMAttentionCopyKV');
  ...
  headValue(
    ...
    Loop({ start: uint(0), end: uint(headDim), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
    ...
    const partner = localDim
      .lessThan(half)
      .select(partnerRaw.mul(invRms).mul(partnerScale).negate(), partnerRaw.mul(invRms).mul(partnerScale));
    const angle = freqIndex
      .lessThan(uint(ropePairCount))
      .select(float(positionNode).mul(ropeFrequencyNode.element(freqIndex)), float(0));
    const rotated = x
      .mul(cos(angle))
      .add(partner.mul(sin(angle)))
      .mul(ropeAttentionFactor);

    return inRotary.select(rotated, x);
  }

  ropeValue(vectorNode: TslNode, headOffset: TslNode, localDim: TslNode, positionNode: TslNode) {
    const { rotaryDim, ropeTheta, ropeFrequencyNode, ropePairCount, ropeAttentionFactor } = this;
    ...
    const inRotary = localDim.lessThan(uint(rotaryDim));
    const freqIndex = localDim.mod(half);
    const partnerIndex = localDim
      .lessThan(half)
      .select(headOffset.add(localDim).add(half), headOffset.add(localDim).sub(half));
    const partner = localDim
      .lessThan(half)
      .select(vectorNode.element(partnerIndex).negate(), vectorNode.element(partnerIndex));
    const angle = freqIndex
      .lessThan(uint(ropePairCount))
      .select(float(positionNode).mul(ropeFrequencyNode.element(freqIndex)), float(0));
    const rotated = x
  ...
  createCopyComputeNode(name: string) {
    const { qSize, kvSize, headDim, position, qkvNode, keyCacheNode, valueCacheNode, workgroupSize } = this;
    ...
    return Fn(() => {
      ...
      .compute(kvSize, [workgroupSize])
      .setName(name);
  }

  createQueryComputeNode(name: string) {
    const { qSize, headDim, position, qkvNode, queryNode, workgroupSize } = this;
    ...
    return Fn(() => {
      ...
      .compute(qSize, [workgroupSize])
      .setName(name);
  }

  createScoreComputeNode(name: string) {
    const {
      headDim,
      headCount,
      kvHeadCount,
    ...
    return Fn(() => {
      ...
      If(index.lessThan(uint(scoreCount)), () => {
        ...
        const windowStart =
          slidingWindow > 0
            ? position
                .add(uint(1))
                .lessThan(uint(slidingWindow))
                .select(uint(0), position.add(uint(1)).sub(uint(slidingWindow)))
            : uint(0);
        ...
        If(token.greaterThanEqual(windowStart).and(token.lessThan(position.add(uint(1)))), () => {
          ...
          Loop({ start: uint(0), end: uint(headDim), type: 'uint', condition: '<' }, ({ i }: { i: TslNode }) => {
            score.addAssign(
              queryNode.element(qOffset.add(i)).mul(keyCacheNode.element(token.mul(uint(kvSize)).add(kvOffset).add(i))),
            );
          });
      ...
      .compute(scoreCount, [workgroupSize])
      .setName(name);
  }

  createSoftmaxComputeNode(name: string) {
    const {
      qSize,
      headDim,
      headCount,
    ...
    return Fn(() => {
      ...
      If(dim.lessThan(uint(qSize)), () => {
        ...
        (Loop as TslNode)(
          { start: windowStart, end: position.add(uint(1)), type: 'uint', condition: '<', name: 'token' },
          ({ token }: { token: TslNode }) => {
