...
async function readChunkedLogits(renderer: Renderer, chunks: LogitChunk[], vocabSize: number): Promise<Float32Array> {
  ...
  const values = await Promise.all(
    chunks.map(async (chunk) => new Float32Array(await renderer.getArrayBufferAsync(chunk.layer.outputAttribute))),
  );
...
class TSLLogitSampler {
  ...
  constructor(chunks: LogitChunk[], options: LogitSamplerOptions = {}) {
    ...
    this.partialTokenNode = storage(this.partialTokenAttribute, 'uint', this.partialCount).setName(
      options.name ? `${options.name}PartialTokens` : 'LLMPartialTokens',
    );
    this.partialScoreNode = storage(this.partialScoreAttribute, 'float', this.partialCount).setName(
      options.name ? `${options.name}PartialScores` : 'LLMPartialScores',
    );
    this.chunkCandidateTokenNode = storage(this.chunkCandidateTokenAttribute, 'uint', this.chunkCandidateCount).setName(
      options.name ? `${options.name}ChunkTokenCandidates` : 'LLMChunkTokenCandidates',
    );
    this.chunkCandidateScoreNode = storage(
      this.chunkCandidateScoreAttribute,
      'float',
      this.chunkCandidateCount,
    ).setName(options.name ? `${options.name}ChunkScoreCandidates` : 'LLMChunkScoreCandidates');
    this.candidateTokenNode = storage(this.candidateTokenAttribute, 'uint', this.candidateCount).setName(
      options.name ? `${options.name}TokenCandidates` : 'LLMTokenCandidates',
    );
    this.candidateScoreNode = storage(this.candidateScoreAttribute, 'float', this.candidateCount).setName(
      options.name ? `${options.name}ScoreCandidates` : 'LLMScoreCandidates',
    );
    this.greedyComputeNodes = [];
    this.computeLevels = [];

    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
      this.greedyComputeNodes.push(
        this.createChunkPartialMaxNode(
          chunks[chunkIndex],
          chunkIndex,
    ...
    for (let rank = 0; rank < this.candidateCount; rank++) {
      ...
      for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        nodes.push(
          this.createChunkCandidateNode(
            chunks[chunkIndex],
            chunkIndex,
            rank,
            options.name
              ? `${options.name}Chunk${chunkIndex}Candidate${rank}`
              : `LLMLogitChunk${chunkIndex}Candidate${rank}`,
          ),
        );
      }

      nodes.push(
        this.createGlobalCandidateNode(
          rank,
          options.name ? `${options.name}Candidate${rank}` : `LLMLogitCandidate${rank}`,
        ),
      );
  ...
  createChunkPartialMaxNode(chunk: LogitChunk, chunkIndex: number, name: string) {
    ...
    return Fn(() => {
      ...
      .compute(chunk.size, [workgroupSize])
      .setName(name);
  }
  ...
  async readCandidates(renderer: Renderer, count: number) {
    ...
    const tokens = new Uint32Array(
      await renderer.getArrayBufferAsync(this.candidateTokenAttribute, null, 0, byteCount),
    );
    const scores = new Float32Array(
      await renderer.getArrayBufferAsync(this.candidateScoreAttribute, null, 0, byteCount),
    );
