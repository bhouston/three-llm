...
class DecoderTSLRunner {
  ...
  constructor(weights: DecoderWeights, options: RunnerOptions = {}) {
    ...
    this.embeddingNode = storage(this.embeddingAttribute, 'float', this.hiddenSize).setName(
      `${weights.architecture}Embedding`,
    );
    ...
    this.prefillCursorNode = storage(this.prefillCursorAttribute, 'uint', 1).setName(
      `${weights.architecture}PrefillCursor`,
    );
    ...
    this.prefillEmbeddingNode = storage(
      this.prefillEmbeddingAttribute,
      'float',
      this.prefillEmbeddingBuffer.length,
    ).setName(`${weights.architecture}PrefillEmbeddings`);
  ...
  buildLayer(block: DecoderBlock, index: number, residualNode: TslNode): DecoderLayer {
    ...
    if (recipe.residual === 'parallel') {
      ...
      const attnProj = new TSLLinear(
        attention.outputNode,
    ...
    renderer.compute(
      computeLogits
        ? sampleCandidateCount > 0
          ? this.sampleComputeNodes(sampleCandidateCount)
          : this.computeNodes
  ...
  prepare(renderer: Renderer) {
    ...
    if (uploadAndReleaseStaticWeights(renderer, this.computeNodes, this.collectStaticWeightAttributes()) === false)
      return;
  ...
  async generate(renderer: Renderer, prompt: string, options: GenerateOptions = {}): Promise<GenerationResult> {
    ...
    return generateAsync(this, prompt, options, {
      ...
      computeToken: (tokenId, position, computeLogits, sampleCandidateCount) =>
        this.computeToken(renderer, tokenId, position, computeLogits, sampleCandidateCount),
      prefillTokens: (inputTokens, start, end, onProgress) =>
        this.prefillTokens(renderer, inputTokens, start, end, onProgress),
