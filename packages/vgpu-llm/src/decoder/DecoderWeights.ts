...
class DecoderWeights {
  ...
  constructor(config: HuggingFaceConfig, tensors: TensorMap, tokenizer: Tokenizer, options: LoaderOptions = {}) {
    ...
    let endOfTextTokenId: number | number[] =
      this.config.eos_token_id ?? tokenizer.endOfTextTokenId ?? this.recipe.endOfTextTokenId ?? 0;
    ...
    this.stopTokenIds = stopTokenIdsFor(
      this.recipe.chatTemplate,
      tokenizer,
      this.recipe.stopTokenIds || [endOfTextTokenId],
    );
    if (this.recipe.chatTemplate !== undefined) {
      this.formatChat = (messages, formatOptions = {}) =>
        formatChatTemplate(this.recipe.chatTemplate!, messages, formatOptions);
    }
  ...
  loadOutputWeight(): Float32Array {
    const useUntiedHead =
      this.architecture !== 'gpt2' && this.hasTensor('lm_head.weight') && this.config.tie_word_embeddings !== true;
  ...
  createBlock(index: number): DecoderBlock {
    ...
    const ropeTheta = isSlidingLayer ? this.localRopeTheta || this.ropeTheta : this.globalRopeTheta || this.ropeTheta;
    const slidingWindow = isSlidingLayer ? this.slidingWindow : this.layerTypes ? 0 : recipe.slidingWindow || 0;
    const qkvBias =
      this.hasMapped('attn_q_bias', index) ||
      this.hasMapped('attn_k_bias', index) ||
      this.hasMapped('attn_v_bias', index)
        ? packBiases([
            this.optionalBias('attn_q_bias', index, qSize),
            this.optionalBias('attn_k_bias', index, kvSize),
            this.optionalBias('attn_v_bias', index, kvSize),
  ...
  embedding(
    tokenId: number,
    position: number,
    target: Float32Array<ArrayBufferLike> = new Float32Array(this.hiddenSize),
  ): Float32Array {
