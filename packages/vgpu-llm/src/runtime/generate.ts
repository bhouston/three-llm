...
function prepareGenerationFromTokens(
  tokens: number[],
  maxTokens: number,
  maxNewTokens: number,
  endOfTextTokenId: number,
...
function planPromptCache(
  cachedTokens: number[] | undefined,
  cachedLogits: Float32Array | null | undefined,
  inputTokens: number[],
  rewindable: boolean,
): PromptCachePlan {
...
function finishGeneration(
  ...
  extra: { reused: number; promptTokens: number; rest: object },
): GenerationResult {
...
async function generateAsync(
  ...
  prompt: string,
  options: GenerateOptions = {},
  controls: GenerateAsyncControls,
