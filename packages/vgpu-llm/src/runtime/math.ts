...
function linear(
  ...
  inputSize: number,
  outputSize: number,
  target = new Float32Array(outputSize),
): FloatVec {
...
function rmsNorm(
  input: FloatVec,
  weight: FloatVec | null | undefined,
  epsilon = 1e-5,
  offsetWeight = false,
  target = new Float32Array(input.length),
): FloatVec {
...
function rmsNormPackedHeads(
  vector: FloatVec,
  headCount: number,
  headDim: number,
  weight: FloatVec | null,
  epsilon: number,
...
function gatedDeltaRuleStep(
  ...
  beta: FloatVec,
  state: FloatVec,
  options: { numVHeads: number; keyDim: number; valueDim: number },
): FloatVec {
...
function sampleTopKCandidates(
  candidates: LogitCandidate[],
  { temperature = 0.8, random = Math.random }: { temperature?: number; random?: () => number } = {},
): number {
