...
6. Wait for required checks and review. Merge feature PRs with a merge commit (squash merges are disabled on this repository); each commit on the branch must already follow Conventional Commits, since merge commits themselves are ignored by commitlint and the release analyzer. Do not merge without maintainer authorization. Merging never publishes; releases are a separate, manually dispatched step (below).
...
Use `main` as GitHub's default branch. Protect it with PRs, required CI checks, no force pushes, and no deletion. Required checks are Unit, Browser Unit, E2E, and Contribution policy. Checkpoints are advisory. Merge feature PRs with a merge commit (squash merges are disabled on this repository). GitHub rules must require the checks for failures to actually block merges. The release workflow is manually dispatched from `main` and is available in Actions only once merged into the default branch.
